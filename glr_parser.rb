require 'rockit/graphdrawing'
require 'rockit/version'
require 'rockit/sourcecode_dumpable'
require 'rockit/parsing_ambiguities'
...
class GeneralizedLrParser
  ...
  def to_src(assignToName = nil, nameHash = {})
    ptname = "@@parse_table" + self.object_id.inspect
  ...
  def actor(stack)
    ...
    tokens.each do |token|
      ...
      next unless actions
      #puts "lexer = #{stack.lexer.inspect} (#{stack.lexer.object_id}), token = #{token.inspect}, actions = #{actions.inspect}"
